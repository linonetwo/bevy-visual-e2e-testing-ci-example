    ...
    Some(match name {
        ...
        "evaluate_script" => {
            ...
            let result = match send(
                sender,
                |tx| TestMessage::EvaluateScript { script, response: tx },
                TIMEOUT,
            )
            .await
            {
...
fn read_log_file(file_path: &str, lines: u32) -> Vec<LogEntryData> {
    match std::fs::read_to_string(file_path) {
        Ok(content) => {
            ...
            all_lines[start..].iter().map(|l| parse_log_line(l)).collect()
        }
        Err(_) => vec![],
    }
}
...
fn parse_log_line(line: &str) -> LogEntryData {
    ...
    if parts.len() >= 3 {
        ...
        if let (Some(start), Some(end)) = (line.find('['), line.find(']')) {
            ...
            let message = line[end + 1..].trim_start_matches([' ', '-']).trim().to_string();
            return LogEntryData { timestamp, level, message };
        }
    }
    LogEntryData {
